...
{
	std::cout << "ClapTrap with default constructor (no named) called" << std::endl;
}
...
void ClapTrap::takeDamage(unsigned int amount)
{
	if (this->_hitPoints && this->_energyPoints)
	{
		...
		std::cout << *this << " takes " << amount << " points of damage!" << std::endl;
	}
	else 
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	{
		std::cout << *this << " has no energy points left to repair!" << std::endl;
		return;
	}
	this->_hitPoints += amount;
	this->_energyPoints -= 1;
	std::cout << *this << " repaired with " << amount << " hit points!" << std::endl;
}